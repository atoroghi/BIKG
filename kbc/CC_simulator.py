...
if __name__ == '__main__':
    ...
    #cov_vars = [1e5]
    #cov_targets = [1e5]
    cov_anchors = [1e-4,1e-3,1e-2, 1e-1,1e1,1e2,1e3,1e4]
    cov_vars = [1e-4,1e-3,1e-2, 1e-1,1e1,1e2,1e3,1e4]
    cov_targets = [1e-4,1e-3,1e-2, 1e-1,1e1,1e2,1e3,1e4]

    #cov_anchors = [1e-2, 1e-1,1e1, 1e2]
    #cov_vars = [1e-2, 1e-1,1e1, 1e2]
    #cov_targets = [1e-2, 1e-1,1e1, 1e2]
    experiment_name = '2_2_conditional_fixed'
    chain_type = '2_2'
    ...
    for cov_anchor in cov_anchors:
        for cov_var in cov_vars:
            for cov_target in cov_targets:
                ...
                command = ["python3", "-m", "kbc.cqd_beam_bpl", "data/Movielens_twohop", "--model_path", "models/Movielens_twohop-SimplE-model-rank-50-epoch-30-1687217986.pt", "--dataset", "Movielens_twohop", "--candidates", "3", "--quantifier", "marginal_ui", "--cov_target", str(cov_target), "--cov_var", str(cov_var), "--cov_anchor", str(cov_anchor), "--chain_type", str(chain_type)]
