...
def run(kbc_path, dataset_hard, dataset_name, t_norm='min', candidates=3,
 scores_normalize=0, kg_path=None, explain=False, user_likes =None, id_ent =None,
  quantifier=None, valid_heads=None, valid_tails=None):
    ...
    for exp in experiments:
        metrics = answer(kbc_path, dataset_hard, t_norm, exp, candidates, scores_normalize, kg_path, explain, user_likes, id_ent, quantifier=quantifier, valid_heads=valid_heads, valid_tails=valid_tails)
...
def answer(kbc_path, dataset_hard, t_norm='min', query_type=QuerDAG.TYPE1_2, candidates=3, scores_normalize = 0, kg_path=None, explain=False, user_likes=None, id_ent=None, quantifier=None, valid_heads=None, valid_tails=None):
    # takes each query chain, creates instruction on what type it is, and replaces each entity with its embedding
    env = preload_env(kbc_path, dataset_hard, query_type, mode='hard', kg_path=kg_path, explain=explain, valid_heads=valid_heads, valid_tails=valid_tails, id_ent=id_ent)
...
##########
    ...
    metrics = evaluate_existential(env, scores, user_likes, id_ent)
...
if __name__ == "__main__":
    ...
    id_ent = pickle.load(open(osp.join(args.path, 'ent_id.pickle'), 'rb'))
    id_rel = pickle.load(open(osp.join(args.path, 'rel_id.pickle'), 'rb'))
    ...
    run(args.model_path, data_hard,
        ...
        kg_path=args.path, explain=args.explain, user_likes=user_likes, id_ent=id_ent,
